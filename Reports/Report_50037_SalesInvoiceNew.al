report 50037 "SalesInvoice GST"
{
    DefaultLayout = RDLC;
    RDLCLayout = './SalesInvoice_GST.rdl';
    Caption = 'Sales - Invoice New';
    ...
    dataset
    {
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            column(ForDEShawLbl; ForDEShawLbl)
            {

            }
            column(DirectorLbl; DirectorLbl)
            {

            }
            column(AmountInwordsVar; AmountInwordsVar)
            {

            }
            column(AmountCustomer; AmountCustomer)
            {

            }
            column(Description_State; StateRec.Description)
            {
            ...
            dataitem(CopyLoop; Integer)
            {
                ...
                dataitem(PageLoop; Integer)
                {
                    ...
                    column(CompanyInfo_add; CompanyInfo."Address 2")
                    {
                    ...
                    column(GSTPerVar1; GSTPerVar[1])
                    {

                    }
                    column(GSTPerVar2; GSTPerVar[2])
                    {

                    }
                    column(GSTPerVar3; GSTPerVar[3])
                    {

                    }
                    column(GSTPerVar4; GSTPerVar[4])
                    {

                    }
                    column(GSTPerVar5; GSTPerVar[5])
                    {

                    }
                    column(GSTAmountGvar1; GSTAmountGvar[1])
                    {

                    }
                    column(GSTAmountGvar2; GSTAmountGvar[2])
                    {

                    }
                    column(GSTAmountGvar3; GSTAmountGvar[3])
                    {

                    }
                    column(GSTAmountGvar4; GSTAmountGvar[4])
                    {

                    }
                    column(GSTAmountGvar5; GSTAmountGvar[5])
                    {
                    ...
                    dataitem("Sales Invoice Line"; "Sales Invoice Line")
                    {
                        ...
                        //B2BESGON14Dec2021 ++
                        column(CESSPerVar; CESSPerVar)
                        {
                        }
                        column(IGSTPerVar; IGSTPerVar)
                        {

                        }
                        column(CGSTPerVar; CGSTPerVar)
                        {

                        }
                        column(SGSTPerVar; SGSTPerVar)
                        {
                        ...
                        begin
                            ...
                            TotalAmountInclVAT += "Line Amount" + +TCSAmt;
                            TotalPaymentDiscountOnVAT += -("Line Amount" - "Inv. Discount Amount" - "Amount Including VAT");
                            SNOvar += 1;
                            //B2BESG>>
                            TotalAmountInclVAT := ROUND(TotalAmountInclVAT, 0.01);
                            ReportCheck.InitTextVariable;
                            ReportCheck.FormatNoText(NoText, TotalAmountInclVAT, "Sales Invoice Header"."Currency Code");
                            AmountInwordsVar := NoText[1];
                            //B2BESG<<
            ...
            begin
                ...
                /*CompanyAddr[3] := CompanyInfo1."Address 2";
                CompanyAddr[4] := CompanyInfo1.City;
                CompanyAddr[5] := CompanyInfo1."Post Code";*/
                ...
                ShowShippingAddr := true;//"Sell-to Customer No." <> "Bill-to Customer No.";
                                         // for i := 1 TO ArrayLen(ShipToAddr) do


                ShipToAddr[1] := "Sales Invoice Header"."Bill-to Name" + "Sales Invoice Header"."Bill-to Name 2";
                ShipToAddr[2] := "Bill-to Address";


                ShipToAddr[3] := "Bill-to Address 2";


                ShipToAddr[4] := "Bill-to City" + ',' + "Bill-to Post Code";
                ShipToAddr[5] := "Bill-to County";


                ShipToAddr[6] := 'PAN : ' + cust."P.A.N. No.";
                ShipToAddr[7] := 'GST : ' + cust."GST Registration No.";
    ...
    var
        ...
        PrevGSTPerVar: Decimal;
        GSTPerVar: array[5] of Decimal;
        AmountInwordsVar: Text[200];
        NoText: array[2] of Text;
        TotalGSTAmount: Decimal;
        ReportCheck: Codeunit 50006;
        TotalNoText: array[2] of Text;
        GSTAmountGvar: array[5] of Decimal;
        ...
        SNoLBl: Label 'S.No.';
        HSNCodeLBL: Label 'HSN Code';
        RateLBL: Label 'Rate';
        QtyLBL: Label 'QTY';
        CGSTPerLBl: label 'CSGT%';
        SGSTPerLBL: Label 'SGST%';
        IGSTPerLBL: Label 'IGST%';
        SubTotalLBL: Label 'Sub Total';
        TaxInvoiceLbl: Label 'Tax Invoice';
        ForDEShawLbl: Label 'For D. E. Shaw India Private Limited';

        DirectorLbl: Label 'Director';
        AmountInwords: Label 'Amount In Words';
    ...
    begin
        ...
        DetailedGSTLedgerEntry.SetCurrentKey("GST %");
        DetailedGSTLedgerEntry.SetRange("Document No.", SalesInvoiceHeader."No.");
        DetailedGSTLedgerEntry.SetRange("Entry Type", DetailedGSTLedgerEntry."Entry Type"::"Initial Entry");
        DetailedGSTLedgerEntry.SetFilter("GST Component Code", '<>%1', 'CESS');
        if DetailedGSTLedgerEntry.FindSet() then
            repeat

                IF PrevGSTPerVar <> DetailedGSTLedgerEntry."GST %" THEN begin
                    i += 1;
                    PrevGSTPerVar := DetailedGSTLedgerEntry."GST %";
                    GSTPerVar[i] := DetailedGSTLedgerEntry."GST %";

                    DetailedGSTLedgerEntry2.Reset();
                    DetailedGSTLedgerEntry2.SetRange("Document No.", DetailedGSTLedgerEntry."Document No.");
                    DetailedGSTLedgerEntry2.SetFilter("GST Component Code", '<>%1', 'CESS');
                    DetailedGSTLedgerEntry2.SetFilter("GST %", '%1', GSTPerVar[i]);
                    if (DetailedGSTLedgerEntry."GST Component Code" = 'CGST') or (DetailedGSTLedgerEntry."GST Component Code" = 'SGST') then begin
                        DetailedGSTLedgerEntry2.SetFilter("GST Component Code", DetailedGSTLedgerEntry."GST Component Code");
                        IGSTCap := CGSTLbl;
                    ...
                    if DetailedGSTLedgerEntry2.FindSet() then
                        repeat
                            ...
                            if SalesInvoiceHeader."Currency Code" <> '' then
                                GSTAmountGvar[i] += Round((Abs(DetailedGSTLedgerEntry2."GST Amount") * SalesInvoiceHeader."Currency Factor"), GetGSTRoundingPrecision(DetailedGSTLedgerEntry."GST Component Code"))
                            else
                                GSTAmountGvar[i] += Abs(DetailedGSTLedgerEntry2."GST Amount")

                        until DetailedGSTLedgerEntry2.Next() = 0;
                end;
            until (DetailedGSTLedgerEntry.Next() = 0) or (i = 5);
...
*/
        ...
        TotalGSTAmount := TotalAmount + CGSTAmt + IGSTAmt + SGSTAmt + CessAmt;
        TotalAmountInclVAT += CGSTAmt + SGSTAmt + IGSTAmt + CessAmt;
