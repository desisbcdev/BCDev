report 50003 AMCOrder3
{
    UsageCategory = ReportsAndAnalysis;
    ApplicationArea = all;
    DefaultLayout = RDLC;
    RDLCLayout = './AMCOrder3.rdl';
    Caption = 'AMC Order3';
    ...
    dataset
    {
        dataitem("Purchase Header"; "Purchase Header")
        {
            ...
            { }
            column(TotalPriceValue1; TotalPriceValue1)
            { }
            column(ForDeshawCap; ForDeshawCap)
            { }
            column(ForDalcinCap; ForDalcinCap)
            { }
            column(ByCap; ByCap)
            { }
            column(LocationCap; LocationCap)
            { }
            column(ModelNoCap; ModelNoCap)
            { }
            ...
            begin
                ...
                if "Currency Code" = '' then
                    TotalPriceValue1 := StrSubstNo(TotalPriceCap1, 'INR')
                else
                    TotalPriceValue1 := StrSubstNo(TotalPriceCap1, "Currency Code");
                if "Currency Code" = '' then
    ...
    var
        ...
        TotalPriceValue1: Text;
        TotalPriceCap: Label 'Total(%1)';
        ByCap: Label 'By:';
        TotalPriceCap1: Label 'Total Amount %1';
        ...
        ForDeshawCap: Label 'For D. E. Shaw India Private Limited.';
        ForDalcinCap: Label 'For Dailcin Airconditioning India Pvt Ltd';
        ServiceDurationCap: Label 'Service Duration(Months)';
        QuantityCap: Label 'Qty';
        UnitPriceCap: Label 'Unit Rate %1';
        UnitPriceValue: Text;
        BoqAppQtyCap: Label 'BOQ Approx. Qty';
        UomCap: label 'UOM';
        LocationCap: Label 'Location';
        ModelNoCap: Label 'Model Numbers';
        SerialNoCap: Label 'Serial Numbers';
