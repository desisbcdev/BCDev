codeunit 50002 "Purch.-Post Cost AllocationNew"
{
    ...
    begin
        with PurchHeader do begin
            ...
            // CheckOverReceiptApproval(PurchHeader);//UPG1.1
        end;
    ...
    begin
        with PurchLine do begin
            ...
            if (Type <> Type::" ") and ("Qty. to Invoice" <> 0) then begin
                ...
                //CA >>
                if PurchLine."Cost Allocation Rule" <> '' then
                    SplitInvPostingBuffer(PurchHeader, PurchLine, PurchLineACY, TempInvoicePostBuffer, InvoicePostBuffer)
                else
    ...
    procedure PurchPost(var PurchaseHeader: Record "Purchase Header")
    var
        PurchLine: Record "Purchase Line";
    begin
        PurchLine.Reset();
        ...
        PurchLine.SetFilter("Cost Allocation Rule", '<>%1', '');
        if PurchLine.FindFirst() then begin
            CODEUNIT.RUN(CODEUNIT::"Purch.-Post Cost AllocationNew", PurchaseHeader);
            Error('');
        end;
    end;

    [EventSubscriber(ObjectType::Codeunit, 92, 'OnAfterConfirmPost', '', false, false)]
    procedure PurchPostandPrint(PurchaseHeader: Record "Purchase Header")
    var
        PurchLine: Record "Purchase Line";
        PurchSetup: Record "Purchases & Payables Setup";
    ...
    procedure PurchasePostPreview(Subscriber: Variant; RecVar: Variant)
    var
        PurchaseHeader: Record "Purchase Header";
        RecRef: RecordRef;
        PurchPost: Codeunit "Purch.-Post Cost AllocationNew";
        PostingPreviewEventHandler: Codeunit "Posting Preview Event Handler";
        PreviewExitStateErr: Label 'The posting preview has stopped because of a state that is not valid.';
        PreviewModeErr: TextConst ENU = 'Preview mode.', DAN = 'Preview mode.';
        NothingToPostMsg: Label 'There is nothing to post.';
        ErrorMessageHandler: Codeunit "Error Message Handler";
        TempDocumentEntry: Record "Document Entry" temporary;
        GLPostingPreview: Page "G/L Posting Preview";
        PurchaseHeaderTemp: Record "Purchase Header";
        ErrorMessageMgt: Codeunit "Error Message Management";
        Result: Boolean;
    ...
    begin
        ...
        PurchaseHeaderTemp := RecVar;
        PurchaseHeader.RESET;
        PurchaseHeader.SETRANGE("No.", PurchaseHeaderTemp."No.");
        ...
        IF Result OR (GETLASTERRORCALLSTACK = '') THEN
    ...
    local procedure SplitInvPostingBuffer(PurchHeader: Record "Purchase Header"; PurchLine: Record "Purchase Line"; PurchLineACY: Record "Purchase Line"; var TempInvoicePostBuffer: Record "Invoice Post. Buffer" temporary; var InvoicePostBuffer: Record "Invoice Post. Buffer")
    var
        InitPurchLine: Record "Purchase Line" temporary;
        LastPurchLine: Record "Purchase Line";
        CostAllocationLine: Record "Cost Allocation Line";
    ...
    begin
        ...
        LastPurchLine.RESET;
        LastPurchLine.SETRANGE("Document Type", PurchLine."Document Type");
        LastPurchLine.SETRANGE("Document No.", PurchLine."Document No.");
        IF LastPurchLine.FINDLAST THEN
            LineNo := LastPurchLine."Line No." + 10000;
        CostAllocationLine.Reset();
        CostAllocationLine.SetRange("Cost Allocation No.", PurchLine."Cost Allocation Rule");
        CostAllocationLine.SetFilter("Allocation %", '>%1', 0);
        if CostAllocationLine.FindFirst() then begin
            repeat
                InitPurchLine.DeleteAll();
                InitPurchLine.INIT;
                InitPurchLine.TRANSFERFIELDS(PurchLine);
                InitPurchLine."Line No." := LineNo;

                InitPurchLine.Validate("Direct Unit Cost", ((PurchLine."Direct Unit Cost" *
                 CostAllocationLine."Allocation %") / 100) * (PurchLine."Qty. to Invoice" / PurchLine.Quantity));
                TempDimSetEntry.DELETEALL();
                clear(DimMgmt);
                DimSetEntry.RESET;
                DimSetEntry.SETRANGE("Dimension Set ID", PurchLine."Dimension Set ID");
                DimSetEntry.SetFilter("Dimension Code", '<>%1', CostAllocationLine."Dimension Code");
                IF DimSetEntry.FINDSET THEN
                    REPEAT
                        if DimVal.Get(CostAllocationLine."Dimension Code", CostAllocationLine."Dimension Value Code") then;
                ...
                if DimVal.Get(CostAllocationLine."Dimension Code", CostAllocationLine."Dimension Value Code") then begin
                    TempDimSetEntry.INIT;
                ...
                InitPurchLine."Dimension Set ID" := DimMgmt.GetDimensionSetID(TempDimSetEntry);
                DimMgmt.UpdateGlobalDimFromDimSetID(InitPurchLine."Dimension Set ID", InitPurchLine."Shortcut Dimension 1 Code",
                 InitPurchLine."Shortcut Dimension 2 Code");

                InitPurchLine.INSERT(TRUE);
                FillInvoicePostBuffer(PurchHeader, InitPurchLine, PurchLineACY, TempInvoicePostBuffer, InvoicePostBuffer);
                LineNo := LineNo + 10000;
            until CostAllocationLine.Next() = 0;
        end;
    ...
    procedure CarryCostAllocationToGLEntry(VAR GLEntry: Record "G/L Entry"; GenJournalLine: Record "Gen. Journal Line")
    begin
        GLEntry."Cost Allocation Rule" := GenJournalLine."Cost Allocation Rule";
        GLEntry."Fixed Deposit No." := GenJournalLine."Fixed Deposit No.";
        GLEntry."PI Date" := GenJournalLine."PI Date";
        GLEntry."Vendor Invoice Date" := GenJournalLine."Vendor Invoice Date";
    end;

    procedure SplitGenJournal(var Genjournal: Record "Gen. Journal Line")
    var
        InitJournal: Record "Gen. Journal Line";
        LastGenLine: Record "Gen. Journal Line";
        CostAllocationLine: Record "Cost Allocation Line";
        TempDimSetEntry: Record "Dimension Set Entry" temporary;
        DimSetEntry: Record "Dimension Set Entry";
        DimMgmt: Codeunit DimensionManagement;
    ...
    begin
        ...
        LastGenLine.RESET;
        LastGenLine.SETRANGE("Document No.", Genjournal."Document No.");
        IF LastGenLine.FINDLAST THEN
            LineNo := LastGenLine."Line No." + 10000;
        CostAllocationLine.Reset();
        CostAllocationLine.SetRange("Cost Allocation No.", Genjournal."Cost Allocation Rule");
        CostAllocationLine.SetFilter("Allocation %", '>%1', 0);
        if CostAllocationLine.FindFirst() then begin
            repeat
                InitJournal.INIT;
                InitJournal.TRANSFERFIELDS(Genjournal);
                InitJournal."Line No." := LineNo;
                InitJournal.INSERT(TRUE);
                InitJournal.Validate(Amount, (Genjournal.Amount * CostAllocationLine."Allocation %") / 100);
                TempDimSetEntry.DELETEALL();

                DimSetEntry.RESET;
                DimSetEntry.SETRANGE("Dimension Set ID", Genjournal."Dimension Set ID");
                DimSetEntry.SetFilter("Dimension Code", '<>%1', CostAllocationLine."Dimension Code");
                IF DimSetEntry.FINDSET THEN
                    REPEAT
                        if DimVal.Get(CostAllocationLine."Dimension Code", CostAllocationLine."Dimension Value Code") then;
                ...
                InitJournal."Dimension Set ID" := DimMgmt.GetDimensionSetID(TempDimSetEntry);
                DimMgmt.UpdateGlobalDimFromDimSetID(InitJournal."Dimension Set ID", InitJournal."Shortcut Dimension 1 Code",
                InitJournal."Shortcut Dimension 2 Code");
                InitJournal.MODIFY(TRUE);
                LineNo := LineNo + 10000;
            until CostAllocationLine.Next() = 0;
            Genjournal.Delete();
        end;
    ...
    procedure ValidateCostAllocationRule(var Rec: Record "Gen. Journal Line"; var xRec: Record "Gen. Journal Line"; CurrFieldNo: Integer)
    var
        CostAllErr: Label 'Bal. Account No. field must be blank if Cost Allocation Rule is defined';
    begin
        if Rec."Cost Allocation Rule" <> '' then
            Error(CostAllErr);
    end;
    //CA END <<
